...
## Inbox email dan OTP customer

AlebabaStore dapat menerima semua email, termasuk OTP game, melalui Resend
Inbound dan menampilkannya sebagai inbox hanya-baca untuk customer. Admin membuat pool 1–100 alamat acak
...
Anda tahu inbox sudah benar-benar dapat menerima email.

1. Tambahkan subdomain penerima di Resend, misalnya `mail.alebabastore.shop`, lalu
...
Password inbox disiapkan admin saat alamat dibuat dan langsung dapat digunakan.
Setelah pembayaran lunas, email dan password yang sama dikirim bersama detail
akun ke email pribadi customer yang sudah diverifikasi saat checkout. Customer
masuk melalui `/login` menggunakan email AlebabaStore dan password tersebut.
Customer hanya dapat membaca pesan; perubahan password hanya dapat
